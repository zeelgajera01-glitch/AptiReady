...
function getMime() {
  if (!mimePromise) {
    mimePromise = Promise.resolve().then(() => __importStar(require('mime'))).then(mod => {
      const modObj = mod;
      const mime = modObj.default && typeof modObj.default.getType === 'function' ? modObj.default : modObj;
      return mime;
    }).catch(err => {
...
function getPLimit() {
  if (!pLimitPromise) {
    pLimitPromise = Promise.resolve().then(() => __importStar(require('p-limit'))).then(mod => {
      const modObj = mod;
      const pLimit = typeof mod === 'function' ? mod : modObj.default || modObj;
      return pLimit;
    }).catch(err => {
