...
const existingQuestions = JSON.parse(fs.readFileSync(starterPath, 'utf8'));

console.log(`Loaded ${existingQuestions.length} existing starter questions.`);
...
topics.forEach(top => {
  ...
  for (let i = 1; i <= 20; i++) {
    ...
    if (existingMap.has(qId)) {
      allQuestions.push(existingMap.get(qId));
    } else {
      allQuestions.push(generateQuestion(topicId, categoryId, 'easy', i));
    }
  }

  // Medium (21 - 40)
  for (let i = 21; i <= 40; i++) {
    const qId = `q_${topicId}_${i}`;
    if (existingMap.has(qId)) {
      allQuestions.push(existingMap.get(qId));
    } else {
      allQuestions.push(generateQuestion(topicId, categoryId, 'medium', i));
    }
  }
  ...
  for (let i = 41; i <= 60; i++) {
    ...
    if (existingMap.has(qId)) {
      ...
      allQuestions.push(generateQuestion(topicId, categoryId, 'hard', i));
    }
  }
});

function generateQuestion(topicId, categoryId, difficulty, index) {
  const id = `q_${topicId}_${index < 10 ? '0' + index : index}`;
...
// 1. PERCENTAGES (Easy: 1-20, Medium: 21-40, Hard: 41-60)
function genPercentages(id, difficulty, index) {
  if (difficulty === 'easy') {
    ...
    const inc = (index % 4 + 1) * 10; // 10%, 20%, 30%, 40%
    ...
    const diff = orig - netVal;
    const netPct = (diff / orig) * 100;
...
// 2. RATIOS (Easy: 1-20, Medium: 21-40, Hard: 41-60)
function genRatios(id, difficulty, index) {
  if (difficulty === 'easy') {
    ...
    const shareC = (ratioC / (ratioA + ratioB + ratioC)) * total;
...
function genAverages(id, difficulty, index) {
  if (difficulty === 'easy') {
    ...
    const s1 = 40;
    const s2 = 60;
    const avgSpeed = (2 * s1 * s2) / (s1 + s2); // Harmonic mean = 48
...
// 4. PROFIT & LOSS (Easy: 1-20, Medium: 21-40, Hard: 41-60)
...
// 5. TIME & WORK (Easy: 1-20, Medium: 21-40, Hard: 41-60)
function genTimeWork(id, difficulty, index) {
  if (difficulty === 'easy') {
    const daysA = 10;
    const daysB = 15;
    const combinedDays = (daysA * daysB) / (daysA + daysB); // 6
    ...
    const daysA = 12;
    const daysB = 24;
    const combined = (daysA * daysB) / (daysA + daysB); // 8
...
// 6. NUMBER SERIES (Easy: 1-20, Medium: 21-40, Hard: 41-60)
...
// 7. DIRECTIONS (Easy: 1-20, Medium: 21-40, Hard: 41-60)
...
// 8. SYLLOGISMS (Easy: 1-20, Medium: 21-40, Hard: 41-60)
...
// 9. GRAMMAR & VOCABULARY (Easy: 1-20, Medium: 21-40, Hard: 41-60)
function genGrammarVocab(id, difficulty, index) {
  if (difficulty === 'easy') {
    return makeQ(id, 'grammar_vocab', 'verbal', 'easy',
      `Choose the synonym of 'CANDID':`,
      ['Deceitful', 'Frank', 'Secretive', 'Cautious'],
      'b',
      `'Candid' means truthful, straightforward, or frank in expression.`,
      "Think of someone who speaks openly and honestly without hiding facts."
    );
  } else if (difficulty === 'medium') {
...
// 10. READING COMPREHENSION (Easy: 1-20, Medium: 21-40, Hard: 41-60)
