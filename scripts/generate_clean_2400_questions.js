...
topics.forEach(top => {
  ...
  // Easy (1 - 80)
  for (let i = 1; i <= 80; i++) {
    const qId = `q_${topicId}_${i < 10 ? '0' + i : i}`;
...
function genPercentages(id, difficulty, index) {
  if (difficulty === 'easy') {
    ...
    const val = index * 12 + 100;
    const inc = ((index % 6) + 1) * 5;
    const finalVal = val * (1 + inc / 100);
    return makeQ(id, 'percentages', 'quant', 'medium',
      `An item originally priced at $${val} undergoes a ${inc}% price increase. What is its new selling price?`,
      `$${finalVal}`,
    ...
    return makeQ(id, 'percentages', 'quant', 'hard',
      `An employee earning $${orig} per month receives a ${p1}% raise, followed by a ${p2}% salary reduction. What is the employee's final monthly salary?`,
      `$${netVal.toFixed(2)}`,
...
function genRatios(id, difficulty, index) {
  if (difficulty === 'easy') {
    const a = (index % 6) + 1;
    const b = a + (index % 4) + 1;
    const mult = index * 7 + 10;
    const total = (a + b) * mult;
    const shareA = a * mult;
    return makeQ(id, 'ratios', 'quant', 'easy',
      `A total amount of $${total} is split between Alice and Bob in the ratio ${a}:${b}. How much money does Alice receive?`,
      `$${shareA}`,
    ...
    const r2 = 3 + (index % 3);
    const r3 = 5 + (index % 4);
    const mult = index * 8 + 15;
    const total = (r1 + r2 + r3) * mult;
    const shareC = r3 * mult;
    return makeQ(id, 'ratios', 'quant', 'medium',
      `A business profit of $${total} is distributed among partners A, B, and C in the ratio ${r1}:${r2}:${r3}. What is C's share?`,
      `$${shareC}`,
    ...
    const mult = index * 5 + 12;
    const m1 = 3 * mult;
    const m2 = 2 * mult;
    const totalLit = m1 + m2;
    const diff = m1 - m2;
    return makeQ(id, 'ratios', 'quant', 'hard',
      `In a liquid mixture of ${totalLit} liters, the ratio of milk to water is 3:2. How many liters of water must be added to achieve a 1:1 milk-to-water ratio?`,
      `${diff} liters`,
...
function genAverages(id, difficulty, index) {
  if (difficulty === 'easy') {
    ...
    return makeQ(id, 'averages', 'quant', 'easy',
      `Calculate the arithmetic mean of the four numbers: ${n1}, ${n2}, ${n3}, and ${n4}.`,
      `${avg}`,
      [`${avg + 3}`, `${avg + 6}`, `${avg + 9}`],
      index,
...
function genProfitLoss(id, difficulty, index) {
  if (difficulty === 'easy') {
    const cp = index * 20 + 60;
    const profit = index * 4 + 10;
    const sp = cp + profit;
...
function genTimeWork(id, difficulty, index) {
  if (difficulty === 'easy') {
    ...
    return makeQ(id, 'time_work', 'quant', 'easy',
      `Worker A can finish job #${index} in ${da} days, while Worker B takes ${db} days for the same job. How many days will they need if they work together?`,
      `${days} days`,
...
function genDirections(id, difficulty, index) {
  ...
  if (difficulty === 'easy') {
    ...
    const t = times[(index - 161) % times.length];
    const hourDirs = ['South-East', 'South-West', 'North-West', 'North', 'South', 'East', 'West', 'North-East'];
    const ansDir = hourDirs[(index - 161) % hourDirs.length];
