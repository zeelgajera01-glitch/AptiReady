 ...
 */
export const uploadAttempt = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
  ...
  let correctCount = 0;
  questionSnapshots.forEach((snap: any) => {
 ...
 */
export const mutateBookmark = functions.https.onCall(async (data, context) => {
